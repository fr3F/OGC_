...
const User = db.user;
const UserMagasin = db.userMagasin;
let bcrypt = require("bcryptjs");
let fs = require("fs");
const emailService = require("../../../config/environnements/dev/email.config");
...
const { verifierAccesConnecte } = require("../../acces/services/fonctionnalite.service");
const { urlBackOffice } = require("../../../config/environnements/dev/environment");
...
async function setMagasin(body){
    if(!body.idMagasin)
        body.idMagasin = null;
    else{
        await helper.verifierExistence(db.magasin, body.idMagasin, "Magasin");
    }
}
...
async function envoyerMotDePasseMail(email, motDePasse, nouveau = false){
    let subject = nouveau? "Nouveau mot de passe sodim": "Informations de Connexion à la Gestion de Caisse";
    let source;
    ...
    let htmlContent = template({ motDePasse: motDePasse, email: email, url: urlBackOffice});
...
async function createUser(req, res){
    ...
    await User.create(user)
        .then(async (data) => {
            await insertUserMagasin(req.body, data.id)
...
async function insertUserMagasin(body, idUser){
    await UserMagasin.destroy({where: {idUser}});
    if(!Array.isArray(body.idMagasins) || !body.idMagasins.length)
        return;
    const userMagasins = body.idMagasins.map((r)=> {
        return {idUser, idMagasin: r};
    }) 
    await UserMagasin.bulkCreate(userMagasins);
}
...
function traitResultatLogin(user, req, res){
    ...
    // insertHistorique(user.id, "Connexion réussie", null, null).then().catch();
...
async function rechercher(req, res){
    ...
    User.findAndCountAll({
        ...
        limit, offset,
        distinct: true
        ,include:["role", "magasins"]
    })
        .then((data) => {
            data = helper.dataToJson(data);
...
function findOne(req, res){
    let id = req.params.id;
    User.findByPk(id, {include: ["role", "magasins"]})
    .then(async (data) => {
        if (data) {
            data = helper.dataToJson(data);
            setCategorie(data);
    ...
    .catch((err) => {
        // insertHistorique(null, "Récupération utilisateur: " + err.message, null, null, 1, 3).then().catch();
        console.log(err)
        helper.sendErrorMessage(res, "Erreur lors de la récupération de l'utilisateur")
    });
}

function setCategorie(user){
    try{
        if(!user.categorie)
            user.categorie = [];
        else
            user.categorie = JSON.parse(user.categorie);
    }
    catch(err){
        user.categorie = [];
    }
...
async function modifier(req, res){
    ...
    if(req.body.categorie)
        req.body.categorie = JSON.stringify(req.body.categorie);
    User.update(req.body, {
        where: { id: id },
    })
        .then(async (num) => {
            await insertUserMagasin(req.body, id)
            if (num == 1) {
                res.send({
