 const menuServ = require('../services/menu.service');
const foncServ = require('../services/fonctionnalite.service');
const helper = require("../../../helpers/helpers.helper");
const { printAccesPdf } = require('../services/impression.service');
...
const printAccess = async (req, res) => {
    try{
        if(!await foncServ.verifierAccesConnecte(100, req, res))
            return;     
        await printAccesPdf(req.user, res);
        // res.send(data);
    }
    catch(err){
        helper.sendError(res, err);
