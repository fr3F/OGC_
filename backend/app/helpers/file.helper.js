...
function readListFromExcel64(file64OrBuffer, feuille = 0) {
    if (!file64OrBuffer) return [];

    let bufferExcel;

    if (Buffer.isBuffer(file64OrBuffer)) {
        bufferExcel = file64OrBuffer;
    } else if (typeof file64OrBuffer === 'string') {
        const base64 = file64OrBuffer.includes(',') ? file64OrBuffer.split(',')[1] : file64OrBuffer;
        bufferExcel = Buffer.from(base64, 'base64');
    } else {
        throw new Error("Paramètre invalide pour readListFromExcel64");
    }

    const workbook = readerXls.read(bufferExcel, { type: 'buffer' });
    const sheetNamesList = workbook.SheetNames;
    return readerXls.utils.sheet_to_json(workbook.Sheets[sheetNamesList[feuille]]);
}
